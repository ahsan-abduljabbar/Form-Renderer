// XML to JSON Converter (https://github.com/abdmob/x2js)
...
$.fn.dxForm = function(config) {
  ...
  var json = x2js.xml_str2json(input.xml);
  
  // Build Form
  
  var html = "<table class = 'table'>";
  
  $.each(json["form-template"]["fields"]["field"], function (index, field) {
      
    if (field._type == "header") {
    
      html += "<tr>" +
              "  <td colspan = '2'><h2>" + field._label + "</h2></td>" +
              "</tr>";
      ...
      switch (field._type) {
        ...
        case "text":
          ...
          html += "<input type = 'text' name = 'Fields[" + field._name + "]' class = 'form-control' />";
          
          break;
          
        case "select":
        
          html += "<select name = 'Fields[" + field._name + "]' class = 'form-control'>";
          
          $.each(field.option, function (index, option) {
            ...
            switch (typeof option) {
              ...
              case "object":
                ...
                //html += "<option value = '" + option._value + "' */>" + option.__text + "</option>";
                
                html += "<option>" + option.__text + "</option>";
      ...
      if (input.showOptions == false) {
        
        html += "  <input type='hidden' name = 'label[" + field._name + "]' value = '" + field._label + "' />" +  
                "</td>";
      ...
      if (input.showOptions == true) {
          
        html += "<td>" +
                "  <select class = 'form-control' name = 'access[" + input.employeeId + "][" + field._name + "]'>" +
                "    <option value = 'editable'>Editable</option>" +
                "    <option value = 'viewOnly'>View Only</option>" +
                "    <option value = 'hidden'>Hidden</option>" +
                "  </select>" +
                "  <input type='hidden' name = 'label[" + input.employeeId + "][" + field._name + "]' value = '" + field._label + "' />"
                "</td>";
      }
